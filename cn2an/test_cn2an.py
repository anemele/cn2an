...
from .cn2an import C2AMode, convert
...
def test_strict():
    ...
    for key, val in strict_data.items():
        # 符合 strict 模式的数据应当能够通过所有模式的测试
        for mode in C2AMode:
            assert convert(key, mode) == val
...
def test_normal():
    ...
    for key, val in normal_data.items():
        for mode in (C2AMode.NORMAL, C2AMode.SMART):
            assert convert(key, mode) == val

        with pytest.raises(ValueError):
            convert(key, C2AMode.STRICT)
    ...
    for val in error_normal_data:
        for mode in (C2AMode.NORMAL, C2AMode.STRICT):
            with pytest.raises(ValueError):
                convert(val, mode)
...
def test_smart():
    ...
    for key, val in smart_data.items():
        assert convert(key, C2AMode.SMART) == val
        for mode in (C2AMode.NORMAL, C2AMode.STRICT):
            with pytest.raises(ValueError):
                convert(key, mode)
